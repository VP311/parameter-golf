# TurboQuant KV Cache Eval on March 23 Stack

This folder starts from the March 23 `LeakyReLU² + Legal Score-First TTT + Parallel Muon` record stack and adds an alternate eval-only cached decode path with KV-cache compression controls. The dense training path and dense eval path remain intact as the control baseline.

## New Eval Modes

`train_gpt.py` now supports a cache-backed sliding evaluator behind env flags:

```bash
KV_CACHE_EVAL=1
KV_CACHE_QUANT=none|qjl|turboquant
KV_CACHE_BITS=4
KV_CACHE_GROUP_SIZE=16
KV_CACHE_VALUES=fp16|int8|turboquant
KV_CACHE_PREFILL_CHUNK=256
```

Current implementation notes:

- `KV_CACHE_EVAL=0` keeps the original dense sliding and TTT evaluator.
- `KV_CACHE_EVAL=1` enables an eval-only incremental decode path.
- `KV_CACHE_QUANT=qjl` and `KV_CACHE_QUANT=turboquant` currently use the same self-contained base-quantizer-plus-residual-sign codec family for cached keys.
- `KV_CACHE_VALUES=fp16` stores cached values in fp16; `int8` and `turboquant` enable compressed value storage.
- The cache path reconstructs dense K/V tensors for attention rather than using a fused custom kernel.
