...
class Hyperparameters:
    ...
    kv_cache_eval = bool(int(os.environ.get("KV_CACHE_EVAL", "0")))
    kv_cache_quant = os.environ.get("KV_CACHE_QUANT", "none")
    kv_cache_bits = int(os.environ.get("KV_CACHE_BITS", 4))
    kv_cache_group_size = int(os.environ.get("KV_CACHE_GROUP_SIZE", 16))
    kv_cache_values = os.environ.get("KV_CACHE_VALUES", "fp16")
    kv_cache_prefill_chunk = int(os.environ.get("KV_CACHE_PREFILL_CHUNK", 256))
...
@dataclass
class LayerKVCache:
    k: QuantizedKVPayload | None = None
    v: QuantizedKVPayload | None = None
    length: int = 0


@dataclass
class EvalCacheState:
...
def _cache_group_size(group_size: int, dim: int) -> int:
    return max(1, min(group_size, dim))


def _cache_projection_matrix(dim: int, device: torch.device, dtype: torch.dtype) -> Tensor:
    gen = torch.Generator(device="cpu")
    gen.manual_seed(0)
    proj = torch.randn(dim, dim, generator=gen, dtype=torch.float32)
    proj = torch.sign(proj)
    proj[proj == 0] = 1
    return proj.to(device=device, dtype=dtype)


def _quantize_groupwise_base(t: Tensor, bits: int, group_size: int) -> tuple[Tensor, Tensor, tuple[int, ...]]:
    *leading, dim = t.shape
    group = _cache_group_size(group_size, dim)
    groups = (dim + group - 1) // group
    pad = groups * group - dim
    if pad > 0:
        t = F.pad(t, (0, pad))
    flat = t.reshape(-1, groups, group).float()
    levels = max(1, 2 ** max(bits - 1, 1) - 1)
    scale = flat.abs().amax(dim=-1, keepdim=True).clamp_min(1e-8) / levels
    q = torch.clamp(torch.round(flat / scale), -levels, levels).to(torch.int16)
    return q, scale.squeeze(-1).to(torch.float16), tuple(leading + [groups, group])


def _dequantize_groupwise_base(q: Tensor, scale: Tensor, original_shape: tuple[int, ...]) -> Tensor:
    *leading, groups, group = q.shape
    out = q.float() * scale.float().unsqueeze(-1)
    out = out.reshape(*leading, groups * group)
    final_dim = original_shape[-1]
...
def _pack_sign_bits(signs: Tensor) -> Tensor:
    ...
    bit_groups = bits.reshape(*bits.shape[:-1], -1, 8)
    weights = torch.tensor([1, 2, 4, 8, 16, 32, 64, 128], device=bits.device, dtype=torch.uint8)
    return (bit_groups * weights).sum(dim=-1)


def _unpack_sign_bits(packed: Tensor, dim: int) -> Tensor:
    ...
    bits = bits.reshape(*packed.shape[:-1], -1)[..., :dim]
    return bits.mul_(2.0).sub_(1.0)


def quantize_kv_cache_tensor(
    t: Tensor,
    quant_mode: str,
    bits: int,
    group_size: int,
    ...
    if quant_mode == "none":
        return QuantizedKVPayload("fp16", {"value": t.to(torch.float16)}, tuple(t.shape))
    effective_bits = 8 if quant_mode == "int8" else bits
    q, scale, qshape = _quantize_groupwise_base(t, effective_bits, group_size)
    base = _dequantize_groupwise_base(q, scale, tuple(t.shape)).to(t.device)
    payload: dict[str, Tensor] = {
        "q": q,
        "scale": scale,
    }
    if quant_mode in {"qjl", "turboquant"}:
        residual = (t.float() - base.float()).contiguous()
        dim = residual.size(-1)
        proj = _cache_projection_matrix(dim, residual.device, residual.dtype)
        signed = torch.matmul(residual, proj)
        sign_bits = _pack_sign_bits(torch.sign(signed))
        residual_norm = residual.norm(dim=-1).to(torch.float16)
        payload.update({
            "sign_bits": sign_bits,
            "residual_norm": residual_norm,
            "proj": proj.to(torch.float16),
        })
    return QuantizedKVPayload(quant_mode, payload, tuple(t.shape))


def dequantize_kv_cache_tensor(payload: QuantizedKVPayload, dtype: torch.dtype, device: torch.device) -> Tensor:
    if payload.kind == "fp16":
        return payload.data["value"].to(device=device, dtype=dtype)
    q = payload.data["q"].to(device=device)
    scale = payload.data["scale"].to(device=device)
    out = _dequantize_groupwise_base(q, scale, payload.shape).to(device=device, dtype=torch.float32)
    if payload.kind in {"qjl", "turboquant"}:
        dim = payload.shape[-1]
        proj = payload.data["proj"].to(device=device, dtype=torch.float32)
        sign_bits = payload.data["sign_bits"].to(device=device)
        residual_norm = payload.data["residual_norm"].to(device=device, dtype=torch.float32)
        signs = _unpack_sign_bits(sign_bits, dim)
        residual_est = torch.matmul(signs, proj.mT) / math.sqrt(dim)
        residual_est = F.normalize(residual_est, dim=-1) * residual_norm.unsqueeze(-1)
        out = out + residual_est
...
class Rotary(nn.Module):
    ...
    def forward_range(self, start_pos: int, seq_len: int, device: torch.device, dtype: torch.dtype) -> tuple[Tensor, Tensor]:
        total = start_pos + seq_len
        cos, sin = self.forward(total, device, dtype)
        return cos[:, start_pos:total], sin[:, start_pos:total]
...
class CausalSelfAttention(nn.Module):
    ...
    def _dense_attention_cached(self, q: Tensor, k: Tensor, v: Tensor, start_pos: int) -> Tensor:
        qh = q.transpose(1, 2)
        kh = k.transpose(1, 2)
        vh = v.transpose(1, 2)
        ...
        k_len = kh.size(-2)
        ctx = k_len - q_len
        mask = torch.full((q_len, k_len), float("-inf"), device=qh.device, dtype=torch.float32)
        if ctx > 0:
            mask[:, :ctx] = 0
        mask[:, ctx:] = torch.triu(mask[:, ctx:], diagonal=1)
    ...
    def forward_cached(
        ...
        cache: LayerKVCache,
        cache_quant: str,
        cache_value_mode: str,
        cache_bits: int,
        cache_group_size: int,
        start_pos: int,
        v_embed: Tensor | None = None,
        v0: Tensor | None = None,
    ) -> tuple[Tensor, Tensor | None, LayerKVCache]:
        bsz, seqlen, dim = x.shape
        q = F.linear(x, q_w.to(x.dtype)).reshape(bsz, seqlen, self.num_heads, self.head_dim)
        ...
        if self.value_residual and v0 is not None:
            lam = self.vr_lambda.to(dtype=v_new.dtype)
            v_new = lam[0] * v0 + lam[1] * v_new
        q = F.rms_norm(q, (q.size(-1),))
        k_new = F.rms_norm(k_new, (k_new.size(-1),))
        cos_q, sin_q = self.rotary.forward_range(start_pos, seqlen, x.device, q.dtype)
        q = apply_rotary_emb(q, cos_q, sin_q, self.rope_dims)
        k_new = apply_rotary_emb(k_new, cos_q, sin_q, self.rope_dims)
        q = q * self.q_gain.to(dtype=q.dtype)[None, None, :, None]
        k_prev = None if cache.k is None else dequantize_kv_cache_tensor(cache.k, k_new.dtype, k_new.device)
        v_prev = None if cache.v is None else dequantize_kv_cache_tensor(cache.v, v_new.dtype, v_new.device)
        k_total = k_new if k_prev is None else torch.cat([k_prev, k_new], dim=1)
        v_total = v_new if v_prev is None else torch.cat([v_prev, v_new], dim=1)
        y = self._dense_attention_cached(q, k_total, v_total, start_pos)
        if self.use_xsa:
            y = self._xsa_efficient(y, v_new)
        if self.gated_attention:
            gate = torch.sigmoid(self.attn_gate(x)).unsqueeze(-1)
            y = y * gate
        next_cache = LayerKVCache(
            k=quantize_kv_cache_tensor(k_total, cache_quant, cache_bits, cache_group_size),
            v=quantize_kv_cache_tensor(v_total, cache_value_mode, cache_bits, cache_group_size),
            length=k_total.size(1),
        )
        y = y.reshape(bsz, seqlen, dim)
        return F.linear(y, out_w.to(x.dtype)), raw_v, next_cache
...
class SmearGate(nn.Module):
    ...
    def forward_cached(self, x: Tensor, prev_embed: Tensor | None) -> tuple[Tensor, Tensor]:
        g = torch.sigmoid(self.gate.to(dtype=x.dtype))[None, None, :]
        if prev_embed is None:
            prev = torch.zeros_like(x[:, :1])
        else:
            prev = prev_embed.to(dtype=x.dtype)
        x_prev = torch.cat([prev, x[:, :-1]], dim=1)
        return (1 - g) * x + g * x_prev, x[:, -1:, :].detach()
...
class BigramHashEmbedding(nn.Module):
    ...
    def forward_cached(self, token_ids: Tensor, prev_token_ids: Tensor | None) -> tuple[Tensor, Tensor]:
        ...
        if prev_token_ids is None:
            ...
            prev = prev_token_ids.to(torch.int32).reshape(token_ids.size(0), 1)
        ...
        if self.proj is not None:
            h = self.proj(h)
        return h * self.scale.to(dtype=h.dtype), token_ids[:, -1:].detach()
...
class Block(nn.Module):
    ...
    def forward_cached(
        ...
        mix = self.resid_mix.to(dtype=x.dtype)
        x_in = mix[0][None, None, :] * x + mix[1][None, None, :] * x0
        attn_out, raw_v, next_cache = self.attn.forward_cached(
            self.attn_norm(x_in) * self.ln_scale_factor,
            q_w, k_w, v_w, out_w,
            cache=cache,
            cache_quant=cache_quant,
            cache_value_mode=cache_value_mode,
...
class GPT(nn.Module):
    ...
    def init_eval_cache(self, batch_size: int, device: torch.device) -> EvalCacheState:
        return EvalCacheState(
            layer_caches=[LayerKVCache() for _ in range(self.num_layers)],
            pos=0,
            tokens=torch.empty(batch_size, 0, dtype=torch.int64, device=device),
            prev_token_ids=None,
            prev_embed=None,
        )
    def truncate_eval_cache(self, state: EvalCacheState, keep_tokens: int) -> EvalCacheState:
        if keep_tokens <= 0 or state.tokens.numel() == 0:
            state.tokens = state.tokens[:, 0:0]
            state.prev_token_ids = None
            state.prev_embed = None
            state.pos = 0
            state.layer_caches = [LayerKVCache() for _ in range(self.num_layers)]
            return state
        state.tokens = state.tokens[:, -keep_tokens:].contiguous()
        state.pos = keep_tokens
        state.prev_token_ids = state.tokens[:, -1:].contiguous()
        value_mode = "none" if Hyperparameters.kv_cache_values == "fp16" else Hyperparameters.kv_cache_values
        for i, layer_cache in enumerate(state.layer_caches):
            if layer_cache.k is None or layer_cache.v is None:
                continue
            k = dequantize_kv_cache_tensor(layer_cache.k, self.qo_bank.dtype, self.qo_bank.device)[:, -keep_tokens:].contiguous()
            v = dequantize_kv_cache_tensor(layer_cache.v, self.qo_bank.dtype, self.qo_bank.device)[:, -keep_tokens:].contiguous()
            state.layer_caches[i] = LayerKVCache(
                k=quantize_kv_cache_tensor(k, Hyperparameters.kv_cache_quant, Hyperparameters.kv_cache_bits, Hyperparameters.kv_cache_group_size),
                v=quantize_kv_cache_tensor(v, value_mode, Hyperparameters.kv_cache_bits, Hyperparameters.kv_cache_group_size),
                length=k.size(1),
            )
        return state
    ...
    def forward_logits_cached(self, input_ids: Tensor, state: EvalCacheState) -> tuple[Tensor, EvalCacheState]:
        if input_ids.ndim != 2:
            raise ValueError("forward_logits_cached expects (batch, seqlen) input_ids")
        if input_ids.size(1) == 0:
            empty = torch.empty(input_ids.size(0), 0, self.tok_emb.num_embeddings, device=input_ids.device, dtype=self.tok_emb.weight.dtype)
            return empty, state
        n = self.num_layers
        cache_quant = Hyperparameters.kv_cache_quant
        cache_bits = Hyperparameters.kv_cache_bits
        cache_group_size = Hyperparameters.kv_cache_group_size
        cache_value_mode = "none" if Hyperparameters.kv_cache_values == "fp16" else Hyperparameters.kv_cache_values
        x_embed = self.tok_emb(input_ids)
        if self.bigram is not None:
            bigram, next_prev_token = self.bigram.forward_cached(input_ids, state.prev_token_ids)
            x_embed = x_embed + bigram
        else:
            next_prev_token = input_ids[:, -1:].detach()
        x_embed = F.rms_norm(x_embed, (x_embed.size(-1),))
        x, next_prev_embed = self.smear.forward_cached(x_embed, state.prev_embed)
        x0 = x
        v0 = None
        skips: list[Tensor] = []
        ve_cache: dict = {}
        start_pos = state.pos
        new_caches: list[LayerKVCache] = []
        for i in range(self.num_encoder_layers):
            ve = self._get_ve(i, input_ids, ve_cache)
            x, raw_v, layer_cache = self.blocks[i].forward_cached(
                x, x0,
                self.qo_bank[i], self.kv_bank[i], self.kv_bank[n + i],
                self.qo_bank[n + i], self.mlp_up_bank[i], self.mlp_down_bank[i],
                cache=state.layer_caches[i],
                cache_quant=cache_quant,
                cache_value_mode=cache_value_mode,
                cache_bits=cache_bits,
                cache_group_size=cache_group_size,
        ...
        for i in range(self.num_decoder_layers):
            ...
            if skips:
                x = x + self.skip_weights[i].to(dtype=x.dtype)[None, None, :] * skips.pop()
            ve = self._get_ve(bi, input_ids, ve_cache)
            x, _, layer_cache = self.blocks[bi].forward_cached(
                x, x0,
                self.qo_bank[bi], self.kv_bank[bi], self.kv_bank[n + bi],
                self.qo_bank[n + bi], self.mlp_up_bank[bi], self.mlp_down_bank[bi],
                cache=state.layer_caches[bi],
                cache_quant=cache_quant,
                cache_value_mode=cache_value_mode,
                cache_bits=cache_bits,
                cache_group_size=cache_group_size,
        ...
        state.pos += input_ids.size(1)
        state.tokens = torch.cat([state.tokens, input_ids], dim=1)
        state.prev_token_ids = next_prev_token
        state.prev_embed = next_prev_embed
        return self.logit_softcap * torch.tanh(logits_proj / self.logit_softcap), state

# --- Sliding window evaluation ---

def _score_cached_logits(
    logits: Tensor,
    y: Tensor,
    prev: Tensor,
    base_bytes_lut: Tensor,
    ...
    tb = base_bytes_lut[y].to(torch.float64)
    tb += (has_leading_space_lut[y] & ~is_boundary_token_lut[prev]).to(torch.float64)
    return loss_sum, token_count, tb.sum()
...
def eval_val_sliding_cached(
    ...
    base_model.eval()
    prefill_chunk = max(1, args.kv_cache_prefill_chunk)
    with torch.inference_mode():
        for ws in my_windows:
            end = min(ws + seq_len, total_tokens)
            x_full = val_tokens[ws:end].to(dtype=torch.int64, device=device).unsqueeze(0)
            y_full = val_tokens[ws + 1 : end + 1].to(dtype=torch.int64, device=device).unsqueeze(0)
            state = base_model.init_eval_cache(batch_size=1, device=device)
            logits_parts: list[Tensor] = []
            for s in range(0, x_full.size(1), prefill_chunk):
                chunk = x_full[:, s : s + prefill_chunk]
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                    logits_part, state = base_model.forward_logits_cached(chunk, state)
                logits_parts.append(logits_part)
            logits_full = torch.cat(logits_parts, dim=1)
            wlen = logits_full.size(1)
            s = 0 if ws == 0 else max(wlen - stride, 0)
            score_y = y_full[:, s:wlen]
            score_prev = x_full[:, s:wlen]
            inc_loss, inc_tokens, inc_bytes = _score_cached_logits(
                logits_full[:, s:wlen], score_y, score_prev,
                base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
            )
            loss_sum += inc_loss
            token_count += inc_tokens
            byte_count += inc_bytes
    if dist.is_available() and dist.is_initialized():
...
def eval_val_sliding(
    ...
    if args.kv_cache_eval:
        return eval_val_sliding_cached(
            args, base_model, rank, world_size, device,
            val_tokens, base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
            stride=stride, eval_seq_len=eval_seq_len,
        )
    seq_len = eval_seq_len or args.train_seq_len
    total_tokens = val_tokens.numel() - 1
        ...
        with torch.inference_mode():
            if args.kv_cache_eval:
                ...
                for ws in my_windows:
                    ...
                    score_y = y_full[:, s:wlen]
                    inc_loss, inc_tokens, inc_bytes = _score_cached_logits(
                        logits_full[:, s:wlen], score_y, x_full[:, s:wlen],
                        base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
                    )
                    loss_sum += inc_loss
                    token_count += inc_tokens
...
def main() -> None:
    ...
    log0(
        f"kv_cache_eval:enabled={int(args.kv_cache_eval)} quant={args.kv_cache_quant} "
        f"bits={args.kv_cache_bits} group={args.kv_cache_group_size} "
        f"values={args.kv_cache_values} prefill_chunk={args.kv_cache_prefill_chunk}"
    )
