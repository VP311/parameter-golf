# Self-Generated Calibration GPTQ Proof Branch

This branch starts from the March 23 `LeakyReLU² + Legal Score-First TTT + Parallel Muon` stack and keeps the training architecture unchanged. The proof-phase change is only in post-training quantization:

- `QUANT_VARIANT=control` keeps the original mixed int6 export path.
- `QUANT_VARIANT=selfgen_fixed` uses model-generated calibration sequences from a single baseline temperature.
- `QUANT_VARIANT=selfgen_mixed` uses mixed-regime self-generated calibration but no selection.
- `QUANT_VARIANT=selfgen_selected` uses mixed-regime self-generated calibration with score-based subset selection.
- `QUANT_VARIANT=dense` evaluates the saved float checkpoint without quantization.

The script also supports replaying quantization from a saved checkpoint without retraining:

```bash
REPLAY_MODEL_PATH=final_model.pt QUANT_VARIANT=control torchrun --standalone --nproc_per_node=1 train_gpt.py
```

Recommended proof flow on `1xH100`:

1. Train once and save `final_model.pt`
2. Replay `dense`
3. Replay `control`
4. Replay `selfgen_fixed`
5. Replay `selfgen_mixed`
6. Replay `selfgen_selected`

Useful env vars:

```bash
QUANT_VARIANT=dense|control|selfgen_fixed|selfgen_mixed|selfgen_selected
REPLAY_MODEL_PATH=final_model.pt
CALIB_CANDIDATE_SEQS=96
CALIB_FINAL_SEQS=32
CALIB_SEQ_LEN=2048
CALIB_TEMPS=0.4,0.8,1.1
CALIB_GEN_BATCH=8
CALIB_SAMPLE_ROWS=1024
CALIB_SEED=2026
```
